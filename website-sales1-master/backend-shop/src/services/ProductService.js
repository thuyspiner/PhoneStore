    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (filter) {
                ...
                if (label === 'type') {
                    const totalTypeFilteredProducts = await Product.count({ [label]: { '$regex': filter[1] } });
                    resolve({
                        status: 'OK',
                        message: 'Success',
                        data: allObjectFilter,
                        total: totalTypeFilteredProducts,
                        pageCurrent: Number(page + 1),
                        totalPage: Math.ceil(totalTypeFilteredProducts / limit)
                    })
                }else{
                    resolve({
                        status: 'OK',
